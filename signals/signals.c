...
/*
🌀​sigint_handler
Questo è il gestore personalizzato per SIGINT quando 
la shell è nel prompt, cioè durante la readline.
Cosa fa:
...
Importantissimo: tutto quello che fai in un signal handler 
deve essere async-signal-safe, quindi:
❌ niente printf
❌ niente malloc / free
...
🌀​setup_signals_interactive
Imposta il comportamento dei segnali quando la shell 
aspetta input dell’utente (interattiva).
SIGINT (Ctrl-C) → gestito da sigint_handler
Comportamento identico a bash: cancella linea e mostra un 
nuovo prompt.
...
SIGINT → default
SIGQUIT → default
Serve perché i comandi lanciati dalla shell devono comportarsi 
come normali programmi Linux.

Esempio:
se lanci cat e premi Ctrl-C, deve terminare il processo figlio,
non la shell.

🌀​setup_signals_parent_exec
Usato dal padre mentre aspetta la terminazione dei 
figli (dopo fork, prima di waitpid).
Durante l’esecuzione dei comandi:
-La shell non deve reagire ai segnali → i figli reagiscono 
al posto suo.
